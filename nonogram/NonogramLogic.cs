...
namespace nonogram
{
    public class NonogramLogic
    {
        ...
        public int[,] GenerateRandomSolution(int rows, int cols, double fillProbability = 0.3)
        {
            ...
            for (int r = 0; r < rows; r++)
                if (!RowHasOne(grid, r))
                    grid[r, rand.Next(cols)] = 1;

            for (int c = 0; c < cols; c++)
                if (!ColumnHasOne(grid, c))
                    grid[rand.Next(rows), c] = 1;
        ...
        public bool CheckSolution(int[,] userGrid, int[,] correctGrid)
        {
            int rows = correctGrid.GetLength(0);
            int cols = correctGrid.GetLength(1);
